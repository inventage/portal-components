...
describe('<portal-card>', () => {
  it('has a default title "I am a glorious title', async () => {
    const el: PortalCard = await fixture(html`<portal-card></portal-card>`);

    expect(el.title).to.equal('I am a glorious title');
  });

  it('can override the title via attribute', async () => {
    const el: PortalCard = await fixture(html`<portal-card title="attribute title"></portal-card>`);

    expect(el.title).to.equal('attribute title');
  });

  it('can override the title via property', async () => {
    const el: PortalCard = await fixture(html`<portal-card .title=${'property title'}></portal-card>`);
  ...
  it('triggers "side-changed" event when sides change', async () => {
    ...
    const el: PortalCard = await fixture(html`<portal-card @side-changed="${changedSpy as EventListener}"></portal-card>`);
  ...
  it('displays default slot elements', async () => {
    ...
    const slotNodes = defaultSlot!.assignedNodes().filter(node => node.nodeType === Node.ELEMENT_NODE) as HTMLElement[];
